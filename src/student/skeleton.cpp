...
Vec3 closest_on_line_segment(Vec3 start, Vec3 end, Vec3 point) {
    ...
    if (t <= 0) return start; // far left
    else if (t >= 1) return end; // far right
    else return t*se + start; // in between
}
...
Mat4 Joint::joint_to_posed() const {
    ...
    Mat4 m = Mat4::euler(pose);
    // Mat4 m;
    const Joint* j = this;
    while (!j->is_root()) {
        j = j->parent;
        m = Mat4::euler(j->pose) * Mat4::translate(j->extent) * m;
    }
    return m;
}
...
void Joint::compute_gradient(Vec3 target, Vec3 current) {
    ...
    // recursively upward in the heirarchy. Each call should store the result
...
void Skeleton::step_ik(std::vector<Skeleton::IK_Handle*> active_handles) {
